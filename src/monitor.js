...
class Monitor {
  constructor (parent, monitor) {
    ...
    this.root.className = 'sc-monitor-root';
...
class VariableMonitor extends Monitor {
  constructor (parent, monitor) {
    ...
    if (typeof this.value === "object" && this.value instanceof Object && !Array.isArray(this.value)) {
      ...
      this.inner.className = 'sc-monitor-inner';

      this.valueRow = document.createElement('div');
      this.valueRow.className = 'sc-monitor-row';

      this.label = document.createElement('div');
      this.label.className = 'sc-monitor-label';
      this.label.textContent = this.getLabel();

      this.valueElement = document.createElement('div');
      this.valueElement.className = 'sc-monitor-value sc-monitor-value-color';
      ...
      if (this.mode === 'slider') {
        ...
        this.sliderRow.className = 'sc-monitor-row';

        this.slider = document.createElement('input');
        this.slider.className = 'sc-monitor-slider';
...
class Row {
  constructor (monitor) {
    ...
    this.root.className = 'sc-monitor-row-root';

    this.indexEl = document.createElement('div');
    this.indexEl.className = 'sc-monitor-row-index';

    this.valueOuter = document.createElement('div');
    this.valueOuter.className = 'sc-monitor-row-value-outer';

    this.editable = this.monitor.editable;
    if (this.editable) {
      this.valueInner = document.createElement('input');
      this.valueInner.tabIndex = -1;
      this.valueInner.className = 'sc-monitor-row-value-inner';
      ...
      this.deleteButton.className = 'sc-monitor-row-delete';
  ...
  _onclickinput () {
    ...
    this.locked = true;
    this.root.classList.add('sc-monitor-row-value-editing');
  ...
  unfocus () {
    if (this.locked) {
      this.locked = false;
      this.valueInner.readOnly = true;
      this.root.classList.remove('sc-monitor-row-value-editing');
    }
  }
}
...
class ListMonitor extends Monitor {
  constructor (parent, monitor) {
    ...
    if (this.editable) {
      ...
      this.addButton.className = 'sc-monitor-list-add';
